...
export class TaxiiConnect {
     ...
     */
    constructor(url: string, user: string, password: string, timeout: number) {
    ...
    timeoutPromise(promise: Promise<unknown>, timeout: number, error: any) {
    ...
    // original code from: https://github.com/jkomyno/fetch-timeout
    fetchTimeout(url: RequestInfo, options: RequestInit | undefined, timeout: number, error: string) {
     ...
     */
    async asyncFetch(path: string, config: any, filter?: any) { //CHANGED
     ...
     */
    async fetchThis(path: string, options: { flag: boolean; cache: any; }, filter?: string | undefined, config?: undefined) { //CHANGED
     ...
     */
    static withoutLastSlash(url: string) {
     ...
     */
    static withLastSlash(url: string) {
     ...
     */
    static asQueryString(filter: { [x: string]: string | number | boolean; }) {
...
export class Server {
     ...
     */
    constructor(path: any, conn: TaxiiConnect) {
     ...
     */
    static isEmpty(obj: { constructor?: any; }) {
     ...
     * @returns {Promise} the Array of api roots information objects
     */
    async _getApiRoots(discovery: { api_roots: { map: (arg0: (url: any) => Promise<void>) => readonly [unknown, unknown, unknown, unknown, unknown, unknown, unknown, unknown, unknown, unknown]; }; }, apiRootMap?: Map<any, any> | undefined) { //CHANGED
        if (!this.apiOptions.flag) {
            // clear the cache
            this.apiOptions.cache = [];
            // fetch all the api_roots in parallel
            await Promise.all(discovery.api_roots.map(async (url: any) => {
            ...
            this.apiOptions.cache = this.apiOptions.cache.filter((element: any | undefined) => (element !== undefined && !Server.isEmpty(element)));
...
export class Collections {
     ...
     */
    constructor(api_root_path: any, conn: TaxiiConnect) {
     ...
     */
    async collections(range?: string | undefined) { //CHANGED
...
export class Collection {
     ...
     * @param {TaxiiConnection} conn - a TaxiiConnection class instance.
     */
    constructor(collectionInfo: { id: string; }, api_root_path: string, conn: TaxiiConnect) {
     ...
     */
    async getObjects(filter: string, range: string | undefined) {
     ...
     */
    async getObject(obj_id: string, filter: any) {
        ...
        .then(bundle => bundle.objects.find((obj: { id: any; }) => obj.id === obj_id) ))));
    }
     ...
     */
    async getManifests(filter: any, range?: string | undefined) {
        var theConfig = this.conn.getConfig;
        if (range !== undefined) {
            theConfig = {
                'method': 'get',
     ...
     */
    async getManifest(obj_id: any, filter: any) {
        return await (this.getManifests(filter).then(objects => objects.find((obj: { id: any; }) => obj.id === obj_id)));
    }
...
export class Status {
     ...
     * @param {TaxiiConnection} conn - a TaxiiConnection class instance.
     */
    constructor(api_root_path: any, status_id: string, conn: TaxiiConnect) {
        this.api_root_path = TaxiiConnect.withLastSlash(api_root_path);

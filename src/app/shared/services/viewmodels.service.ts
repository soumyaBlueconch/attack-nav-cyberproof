...
import { DataService,Technique, Tactic, Matrix, Domain } from "../services/data.service";
...
export class ViewModelsService {
   ...
   */
  newViewModel(name: string, domainID: string|any) {
  ...
  layerLayerOperation(domainID: string, scoreExpression: string, scoreVariables: Map<string, ViewModel>, comments: ViewModel, gradient: ViewModel, coloring: ViewModel, enabledness: ViewModel, layerName: string, filters: ViewModel, legendItems: ViewModel): ViewModel {
      ...
      function inherit(inherit_vm: ViewModel, fieldname: string) {
          ...
          inherit_vm.techniqueVMs.forEach(function(inherit_TVM:any) {
              let tvm:any = result.hasTechniqueVM_id(inherit_TVM.technique_tactic_union_id) ? result.getTechniqueVM_id(inherit_TVM.technique_tactic_union_id) : new TechniqueVM(inherit_TVM.technique_tactic_union_id)
...
export class Gradient {
  ...
  deSerialize(rep: string): void {
      ...
      let isColorStringArray = function(check: string | any[]): boolean {
...
export class ViewModel {
  ...
  // Getter
  public getTechniqueVM(technique: Technique, tactic: Tactic): TechniqueVM|any {
  ...
  public getTechniqueVM_id(technique_tactic_id: string): TechniqueVM|any {
  ...
  public highlightedTactic: Tactic |any= null;
  public highlightedTechnique: Technique|any = null;
  ...
  * * @param  {Tactic}  tactic wherein the technique occurs
  ...
  public isTacticSelected(tactic: Tactic) {
      let self = this;
      var result = tactic.techniques.every(function(technique: any) {
  ...
  public calculateAggregateScore(technique: Technique, tactic: Tactic): any {
      ...
      technique.subtechniques.forEach((subtechnique: any) => {
...
export class Filter {
   ...
   */
  // toggleInFilter(filterName: any, value: any): void {
   ...
   */
  // inFilter(filterName: any | number, value: any): boolean {
  ...
  deSerialize(rep: any): void {
      // console.log(rep)
      let isStringArray = function(check: string | any[]): boolean {
          for (let i = 0; i < check.length; i++) {
      ...
      if (rep.platforms) {
          if (isStringArray(rep.platforms)) {
              let backwards_compatibility_mappings:any = { //backwards compatibility with older layers
                  "android": "Android",
                  "ios": "iOS",
              ...
              rep.platforms.forEach(function (platform: string) {
